...
the sender and receiver modules of pathchirp. It reads in the
...
void parse_cmd_line_run(argc,argv)
  int	argc;
     char	*argv[];
{
 ...
 case 'l':/* lowest rate to probe at */
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no lowest rate '-l'.\n");
	   (void) exit (1);
	 }
	 low_rate = atof (*argv);
       }
	 ...
	 *ptr = 0;
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no highest rate '-u'.\n");
	   (void) exit (1);
	 }
	 high_rate = atof (*argv);
       }
	 ...
	 *ptr = 0;
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no average rate '-a'.\n");
	   (void) exit (1);
	 }
	 avg_rate = atof (*argv);
       }
	 ...
	 *ptr = 0;
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no spread_factor '-t'.\n");
	   (void) exit (1);
	 }
	 ...
	 *ptr = 0;
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no packet size given with '-p'.\n");
	   (void) exit (1);
	 }
	 pktsize = atoi (*argv);
       }
	 ...
	 *ptr = 0;
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no Jumbo size given with '-J'.\n");
	   (void) exit (1);
	 }
	 jumbo = atoi (*argv);
       }
	 ...
	 *ptr = 0;
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no destination host given with '-S'.\n");
	   (void) exit (1);
	 }
	 ...
	 *ptr = 0;
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no receiver host given with '-R'.\n");
	   (void) exit (1);
	 }
	...
	break;
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no busy period length '-b'.\n");
	   (void) exit (1);
	 }
	 ...
	 *ptr = 0;
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no decrease factor '-d'.\n");
	   (void) exit (1);
	 }
	 ...
	 *ptr = 0;
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no averaging length '-n'.\n");
	   (void) exit (1);
	 }
 ...
 case 'U':		/* UDP port */
	 ...
	 if (*argv == 0) {
	   (void) fprintf (stderr,
			   "pathchirp_run: no port number given with '-U'.\n");
	   (void) exit (1);
	 }
	 sndPort = atoi (*argv);
       }
	 ...
	 *ptr = 0;
	 ...
	 usage();
      ...
      default:
	 (void) fprintf (stderr,
			 "pathchirp_run: Unknown option '%c'\n", ptr[-1]);
 ...
 /* checking  parameters */
   ...
   if (src_addr.s_addr == 0) {
      (void) fprintf (stderr, "pathchirp_run: %s: unknown sender host\n",
		      hostname);
     usage();
      (void) exit (1);
   }
   ...
   if (dest_addr.s_addr == 0) {
      (void) fprintf (stderr, "pathchirp_run: %s: unknown receiver host\n",
		      rxhostname);
     usage();
      (void) exit (1);
   }
  ...
  if (decrease_factor <=1.0)
     {perror ("pathchirp_run: decrease_factor invalid");
	 (void) exit (1);
     }

  if (duration <0)
     {perror ("pathchirp_run: time duration invalid");
	 (void) exit (1);
     }
  else
    {
  ...
  if (jumbo<1 || jumbo>20)
     {perror ("pathchirp_run: jumbo must be between 1 and 20");
	 (void) exit (1);
     }

  if (busy_period_thresh <=2)
     {perror ("pathchirp_run: busy_period_thresh invalid, must be integer >2");
	 (void) exit (1);
     }

  if (sndPort<1024 ||  sndPort>65535)
     {perror ("pathchirp_run: Port number must be in [1024-65535]");
	 (void) exit (1);
     }
 ...
 /*check if parameters ok*/
  if (pktsize<MINPKTSIZE)
    {
      fprintf(stderr,"pathchirp_run: packet size too small, using minimum sized %d byte packet\n",MINPKTSIZE);
      pktsize=MINPKTSIZE;
    }
  else
    {
      if (pktsize>MAXMESG)
	{
	  fprintf(stderr,"pathchirp_run: packet size too big, using maximum sized %d byte packet\n",MAXMESG);
	  pktsize=MAXMESG;
	}
    }

  if(spread_factor<MINSPREAD)
    fprintf(stderr,"pathchirp_run: packet spread too small, using minimum packet spread %f\n",MINSPREAD); 

  if( low_rate<0.0 || high_rate<0.0 || avg_rate<0.0 || low_rate>high_rate)
    {
      perror("pathchirp_run: probing rates invalid\n"); 
      exit(0);
    }
...
void connect_to_rcv(int argc,char **argv_ptr)
{
 ...
 int nc;
   ...
   if (sotcp < 0) {
      perror("pathchirp_run: tcp socket");
      exit(1);
   }
   ...
   while(num_args<=argc)
	...
	if (write(sotcp,data_snd,100) < 0)
		{
		 perror("pathchirp_rcv_tcp: write");
		 exit(1);
		}
